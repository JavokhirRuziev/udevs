    ...
    <Box sx={wrapperStyles} className="SlideFromTopToBottom">
      <Container>
        <Box sx={containerStyles}>
          <Logo style={{ width: 96, height: 31 }} />
          <Box
            sx={{ display: "flex", alignItems: "center", columnGap: "32px" }}
          >
            {headerCategories?.map((el, index) => (
              <Tooltip
                ...
                title={
                  el?.children && (
                    <Box sx={{ minWidth: el?.name !== "Language" ? 300 : 120 }}>
                      {el?.name !== "Language" && (
                        <Typography
                          variant="subtitle1"
                          p={"0px 24px"}
                      ...
                      {el?.children?.map((e, ind) => (
                        <Box
                          sx={tooltipChildrenStyles(el)}
                          key={ind}
                          onClick={() => handleScroll(e?.id)}
                        >
                          <Box width={el?.name !== "Language" ? 48 : 28}>
                            {e?.icon}
                          </Box>
                          <Typography
                            id="text"
              ...
              >
                <Typography
                  ...
                  onClick={(e) => handleScroll(el?.name)}
                >
                  {el?.name}
