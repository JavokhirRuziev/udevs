...
const Tools = () => {
  ...
  const [programs, setPrograms] = useState<Program[]>(programsArr as Program[]);
    ...
    <Box sx={{ bgcolor: "primary.light" }}>
      <Container>
        <Box sx={{ p: "80px 0px" }}>
          ...
          <FormControl sx={{ mb: "48px" }} className="staggered hidden">
            <RadioGroup row value={selected}>
              {programsNameArr?.map((el) => (
                <FormControlLabel
                  key={el.id}
                  sx={{ ".MuiRadio-root": { color: "primary.main" } }}
                  control={
                    <Radio
                  ...
                  label={<Typography variant="body1">{el.name}</Typography>}
                />
              ))}
            </RadioGroup>
          </FormControl>
